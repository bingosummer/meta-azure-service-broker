...
sqldbOperations.prototype.validateCredentials = function (server, username, password, callback) {
    ...
    var conn = sqlDb.connect(config, function(err) {
      if (err) {
        console.log('sqldb client: validateCredentials: err: %j', err);
        callback(err);
      } else {
        console.log('sqldb client: validateCredentials: succeeded');
        conn.close();
        callback(null);
      }
    });
