...
// Delete old record if a record with the same 'azureInstanceId' but timestamp is older than 60 minutes but lastOperation='provision'
...
Database.prototype.updateServiceInstanceStatus = function(instanceId, status, callback) {
  var params = util.format('status=\'%s\'', status);
  this.updateServiceInstance(instanceId, params, callback);
};
