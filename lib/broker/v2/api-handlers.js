...
Handlers.handleProvisionRequest = function(broker, req, res, next) {
  ...
  var processResponse = function(error, reply, result) {
    if (error) {
      ...
      // Need to delete the inserted record from the database
      ...
      // Need to update result in the inserted record in the database
      broker.db.updateServiceInstanceProvisioningResult(instanceId, result, function(err){
        if (err) {
          // CC will call deprovision
          var msg = util.format('Failed in updating the provision record for (instanceId: %s) in the database: %s', instanceId, err);
          broker.log.error('handleProvisionRequest - %s', msg);
  ...
  // Insert a record with (status:pending, lastOperation: provision) into the database
  broker.db.createServiceInstance(req.params, function(err){
    if (err) {
      broker.log.warn('Failed in inserting the record for (instanceId: %s) into the broker database. It may be caused by that a record with the same azureInstanceId exists.', instanceId);
...
var handleProvisionPollResponse = function(error, reply, result, broker, req, res, next) {
  ...
  if (error) {
    ...
    broker.db.deleteServiceInstance(instanceId, function(err){
      if (err) {
        // CC will call deprovision
        var msg = util.format('Failed in deleting the record (instanceId: %s) from the broker database. Provision Error: %j. DB Error: %s.', instanceId, error, err);
    ...
    broker.db.updateServiceInstanceProvisioningResult(instanceId, JSON.stringify(result), function(err){
      if (err) {
        // CC will call deprovision
        var msg = util.format('Failed in updating the record for (instanceId: %s) in the broker database. DB Error: %s', instanceId, err);
...
var handleDeProvisionPollResponse = function(error, reply, result, broker, req, res, next) {
  ...
  if (error) {
    ...
    if (error.statusCode == HttpStatus.NOT_FOUND) {
      broker.db.deleteServiceInstance(instanceId, function(err){
        if (err) {
          var msg = util.format('Failed in deleting the record for (instanceId: %s) from the broker database. Deprovision Error: %j. DB Error: %s.', instanceId, error, err);
    ...
    broker.db.deleteServiceInstance(instanceId, function(err){
      if (err) {
        // CC will call deprovision
        var msg = util.format('Failed in deleting the record for (instanceId: %s) in the broker database. DB Error: %s', instanceId, err);
...
Handlers.handlePollRequest = function(broker, req, res, next) {
  ...
  broker.db.getServiceInstance(instanceId, function(err, serviceInstance) {
    if (err) {
      var msg = util.format('Failed in finding the record by (instanceId: %s) in broker database. DB Error: %s', instanceId, err);
...
Handlers.handleDeProvisionRequest = function(broker, req, res, next) {
  ...
  var processResponse = function(error, reply, result) {
    ...
    broker.db.updateServiceInstanceLastOperation(instanceId, 'deprovision', function(err){
      if (err) {
        var msg = util.format('Failed in updating the record (instanceId: %s) in broker database. DB Error: %s', instanceId, err);
...
Handlers.handleBindRequest = function(broker, req, res, next) {
  ...
  var processResponse = function(error, reply, result) {
    ...
    broker.db.createServiceBinding(req.params, result, function(err){
      if (err) {
        var msg = util.format('Failed in inserting the record (bindingId: %s) for(instanceId: %s) in broker database. DB Error: %s', bindingId, instanceId, err);
...
Handlers.handleUnbindRequest = function(broker, req, res, next) {
  ...
  var processResponse = function(error, reply, result) {
    ...
    broker.db.deleteServiceBinding(req.params.binding_id, function(err){
      if (err) {
        var msg = util.format('Failed in deleting the record (bindingId: %s) for(instanceId: %s) in broker database. DB Error: %s', bindingId, instanceId, err);
  ...
  async.series([
      ...
      function(callback) {
        broker.db.getServiceBinding(bindingId, function(err, binding) {
          if (err) {
            var msg = util.format('Failed in finding the record by (bindingId: %s) in broker database. DB Error: %s', bindingId, err);
            return callback(msg);
          }
