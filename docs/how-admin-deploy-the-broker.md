...
2. Update `manifest.yml` with your credentials.
  ...
  The contents of `manifest.yml` should be:
  ...
  * `DOCDB_HOSTENDPOINT` and `DOCDB_MASTERKEY`

    These two valuese are to enable the DocumentDB service. Please read [Azure DocumentDB Service](./docs/azure-document-db.md) for more information.

  * `SECURITY_USER_NAME` and `SECURITY_USER_PASSWORD`

    These two values are the username and password of HTTP basic authentication. They are specified by yourself.
...
3. Update `config/default.json`. (Optional)
    ...
    1. `azure` includes the service principal and the Azure DocumentDB Host information.
      ...
      "azure": {
        ...
        "docdbHostendpoint": "your-docdb-host-endpoint",
        "docdbMasterkey": "your-docdb-masterkey"
      },
      ```
